...
\subsection{Subheaps and the subheap relation}
...
\begin{enumerate}
    ...
    \item If $\sigma_L(a)=\sigma_R(a)=a$ for all $a$, then we obtain a two-sided brace (i.e. a radical ring). 
\end{enumerate}
...
\begin{theorem}
\label{thm:ring}
    Let $T$ be a non-empty truss. For every $G(T,e)$ is an associative
    ring with multiplication $ab=[ab,ae,e^2,eb,e]$.
...
\begin{theorem}
    Let $T$ be a non-empty truss and $e\in T$. 
    The following statement hold:
    \begin{enumerate}
        \item Let $\epsilon_R,\epsilon_L\colon T\to T$, with
        $\epsilon_R(a)=[ea,e^2,e]$ and
        $\epsilon_L(a)=[ae,e^2,e]$. Then...
        \item The ring $R(e)$ has multiplication...
        \item The sequence
        \[
        ...
        \]
        is an ideal extension.
        \item $(T,11)=\left(T(R(e),(e,1)\right)$.
    \end{enumerate}
\end{theorem}

\subsection{Ideal extension of rings}
\subsection{Ideal extensions and homothetism}
