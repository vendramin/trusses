...
%We start with some basic definitions from universal algebra. 
...
\begin{example}
    Let $A$ be an affine space. Then $A$ is an abelian heap with the operation $[a,b,c]=a+\overrightarrow{bc}$. 
\end{example}
...
\begin{proof}[Sketch of the proof]
    ...
    To prove 5) use the map $G\to G(H(G),x)$, $a\mapsto ax$. 
\end{proof}

Let us review some properties of heaps. 
...
\begin{enumerate}
    ...
    \item $a=b$ if and only if there exists $c\in H$ such that $[a,b,c]=c$ (or equivalently, 
        if $[a,b,c]=c$ for all $c\in H$). 
\end{enumerate}
\end{theorem}

\begin{proof}[Sketch of the proof]
We first prove 1). Let $a,b,c,d,e\in H$. Then
\begin{align*}
    [a,b,[c,d,e]]=[[a,b,b],b,[c,d,e]]=[[[a,b,b],b,c],d,e]
    =[[a,[b,b,b],c],d,e]...
\end{align*}


Let us prove 2). Let $a,b,c\in H$ and $d=[a,b,c]$.
Then
\[
...
\]

We now prove 3). Clearly, if $a=b$, then $[a,b,c]=[a,a,c]=c$ by Malcev's identity. 
...
give a useful trick that avoids painful calculations. 
Let us do a concrete example. Let $H$ be an abelian heap. Then
\begin{align*}
    [x,y,x,y,x]=...
\end{align*}
