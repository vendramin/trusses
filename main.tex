...
Notes by Leandro Vendramin. Please send comments 
and corrections to \url{Leandro.Vendramin@vub.be}.  
...
\begin{example}
    ...
    Let $m\colon A\times A\to A$, $(x,y)\mapsto xy$, be the multiplication map. 
    If $m$ is bi-affine, 
    then 
    \[
...
\begin{proposition}\
    \begin{enumerate}
        ...
        \item Let $T$ be a skew left truss such that $(T,\cdot)$ is a group. Then 
            $x+y=[x,1,y]$ turns $G(T,1)$ into a skew left brace. 
    \end{enumerate}  
\end{proposition}
