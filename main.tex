...
word is 
\[
(x_1\cdots x_{2n+1})^{\operatorname{op}}=x_{2n+1}\cdots x_2x_1.
\]
On $W(X)$ we define a ternary operation $[-,-,-]$ by \emph{grafting and pruning}:
if $u,v,w\in W(X)$, then $[u,v,w]$ is obtained by removing (or pruning) all 
pairs of consecutive equal letters from $uv^{\operatorname{op}}w$ (obtained by
concatenation (or grafting) of $u$, $v^{\operatorname{op}}$ and $w$. One
proves that $\left(W(X),[-,-,-]\right)$ is a heap. It is the \textbf{free heap} on 
$X$ and it will be denoted by $H(X)$. 

\begin{definition}
    Let $X$ be a set. A \textbf{free heap} on $H(X)$ is a heap with with the following
    property: for any heap $H$ and any map $f\colon X\to H$ there exists a unique
    heap homomorphism $\varphi\colon H(X)\to H$ such that
\[\begin{tikzcd}
	X & {H(X)} \\
	H
...
\begin{example}
        $H(\{x,y\})\simeq H(\Z)$. In fact, $x\mapsto 0$ and
        \[
...
\begin{proposition}
    \begin{enumerate}
        \item If we view a group $(B,+)$ as a heap $H(B)$, then $H(B)$ is a left skew truss.
